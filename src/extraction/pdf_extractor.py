...
class PDFExtractor:
    ...
    def extract_text(self, pdf_path: Path) -> Optional[Dict[str, List[str]]]:
        """Extract text from a single PDF file, preserving page-level information.
        ...
        try:
            ...
            # Store text for each page
            extracted_data = {
                "doc_name": pdf_path.stem,  # Use stem to remove .pdf extension
                "page_count": doc.page_count,
                "pages": []
            }
    ...
    def process_pdfs(self, pdf_files: Optional[List[Path]] = None) -> List[Dict[str, List[str]]]:
        """Process specified PDFs or all PDFs in the input directory if none specified.
        
        Args:
            pdf_files (Optional[List[Path]]): List of Path objects to PDF files to process.
        ...
        if pdf_files is None:
            pdf_files = list(self.input_dir.glob("*.pdf"))  # Non-recursive search
            if not pdf_files:
